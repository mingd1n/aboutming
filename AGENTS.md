...
- **同步方式（已定）**：脚本从 `Axel/` 导出到 `data/generated.json`（**入库**，部署快照）；只读 vault，绝不写入。CI/Vercel 无 `Axel/` 时沿用快照，不失败。**一次 sync 必须带上整篇：`title`（Obsidian 文件名 basename）+ 全文 `content` + 链接解析**——不是只更新入口列表/卡片链接。漏正文或标题仍显示旧名/路径前缀 = 没做完。
- **上站验收（双语 · 标题+正文 · 必做）**：每次 `sync` + 部署后，**`/zh` 与 `/en` 都要验**：
  1. 方向页卡片/标签上的**标题**与 vault 文件名一致（不要残留 `web3学习/` 之类路径前缀）；
  2. 点进 `/zh/notes/...` 与 `/en/notes/...` 能看到**完整正文**（抽查关键段落，不是空页/旧稿）；
  3. UI 壳随语言切换；**vault 长笔记正文默认中文、不硬翻**（英文路由仍打开同一篇中文正文）。
  只验中文站、或只验链接能点开但不管标题/正文 = 没做完。
- **内容处理规则**：
